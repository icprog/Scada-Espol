...
static int drawGraphic1(PARAM *p, int id, DATA *d)
{
  ...
  gBoxWithText  (p, 50, 50, 1050, 550, fontsize, "Tiempo", "Temperatura", NULL);
  ...
  gText         (p, 50, 50-fontsize*2, "Historial", ALIGN_LEFT);
...
static int slotInit(PARAM *p, DATA *d)
{
  ...
  qpwSetTitle(p,qwtPlot1,"Historial");
  ...
  // axes
  qpwSetAxisTitle(p,qwtPlot1,xBottom, "tiempo");
  qpwSetAxisTitle(p,qwtPlot1,yLeft, "temperatura");

  // curves
  qpwInsertCurve(p,qwtPlot1, 0, "temperatura vs tiempo.");
...
static int slotNullEvent(PARAM *p, DATA *d)
{
  ...
  for(int i=0; i<100; i++)
  {
    ...
    d->sin_x[i]=sinf((float) d->x1[ii]+d->phi1);
